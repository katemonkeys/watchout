// (function() {
var axes, enemyData,gameBoard, gameOptions,gameStats,play,render,Player,updateScore,updateBestScore;
...
render = function(enemy_data) {
  var checkCollision, enemies, onCollision, tweenWithCollisionDetection, svgElement;

  enemies = gameBoard.selectAll('circle').data(enemy_data, function(d) {return d.id;});

  svgElement = d3.select('.container').select('svg');
  ...
  return enemies.transition().duration(500).tween('custom', tweenWithCollisionDetection);
};
